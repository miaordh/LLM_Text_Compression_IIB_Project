...
class LLMCodec:
    ...
    def encode(
        ...
        if show_progress:
            iterator = tqdm(iterator, total=len(token_ids), desc="Encode")
        ...
        try:
            with self._invariant_context():
                ...
                if safe_mode:
                    ...
                    if show_progress:
                        iterator = tqdm(iterator, total=target_tokens, desc="Decode")
                    ...
                    for _ in iterator:
                        ...
                        if not (0 <= int(token_id) < vocab_size):
                            print(f"[llm_codec] ERROR: Out-of-range decoded token_id {token_id} at position {idx} (vocab_size={vocab_size})", file=sys.stderr)
                    ...
                    if show_progress:
                        decode_iterator = tqdm(desc="Decode", unit="tok")
